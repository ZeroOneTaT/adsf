/***
 * @Author: ZeroOneTaT
 * @Date: 2023-12-25 17:09:47
 * @LastEditTime: 2023-12-25 17:09:50
 * @FilePath: /adsf/include/utils/scheduler.hpp
 * @Description: 调度器类
 * @
 * @Copyright (c) 2023 by ZeroOneTaT, All Rights Reserved.
 */

#ifndef ADFS_SCHEDULER_HPP_
#define ADFS_SCHEDULER_HPP_
...
namespace adsf
{
    ...
    private:
        ...
        std::list<ScheduleTask> m_tasks;    // 调度器任务队列
        std::string m_name;                 // 调度器名称
    protected:
        std::vector<int> m_threadIds;            // 线程池线程id
        size_t m_threadCount = 0;                // 线程池线程数量
        std::atomic<size_t> m_activeThreads = 0; // 活跃线程数
        std::atomic<size_t> m_idleThreads = 0;   // 空闲线程数
        bool m_stop = false;                     // 调度器是否停止
    };
}

#endif // ADFS_SCHEDULER_HPP_